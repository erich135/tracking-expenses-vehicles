...
const AddRentalIncomePage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const submissionData = {
            ...formData,
            user_id: user.id,
            rental_equipment_id: formData.rental_equipment_id?.id,
        ...
        const { error } = await supabase.from('rental_incomes').insert([submissionData]);

        if (error) {
            toast({ variant: 'destructive', title: 'Error adding income', description: error.message });
        } else {
            toast({ title: 'Success!', description: 'Rental income added successfully.' });
    ...
    return (
        <>
            <Helmet>
                <title>Add Rental Income</title>
                <meta name="description" content="Add a new income record for rental equipment." />
            </Helmet>
...
export default AddRentalIncomePage;